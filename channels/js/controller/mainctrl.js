  ...
  myapp.controller("mainctrl",function($scope,$http,$rootScope,dataShare){
    $(() => {
       ...
       $scope.chan=["makemytrip","tripadvisor","zomato","holidayiq","booking","vipul","madhulika"];
      $scope.csv=[];
      $scope.data={};
      var results=[];
      $scope.channels=[];
...
dataShare.send_channels($scope.channels);
      ...
      $scope.getdata = function(){
       ...
       var get_data= function(callback){ $http.get("https://www.googleapis.com/customsearch/v1?key=AIzaSyCRKBMoMLvdnJXHB3mjYNF-VPRw5b6Y-50&cx=008935209388523953567:1zh7j-szwa4&q="+hot+" "+loc+" "+$scope.channels[i]).then(function(response){
           console.log("loc",loc);
    ...
    $scope.channels1 = $scope.channels.toString();
        $http.post('/link', {'hotel':$scope.get_e,'location':$scope.get_loc,'chan':$scope.channels1,'time':n,'date':date}).then(
      function(resp) {
        console.log(resp);
      },
      function(errorResp) {
        console.log(errorResp);
           ...
           // $http.post(
           //   "insert.php",
           //   {'hotel':$scope.get_e,'location':$scope.get_loc,'chan':$scope.channels1,'time':n,'date':date}
